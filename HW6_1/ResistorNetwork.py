...
class ResistorNetwork():
    ...
    def AnalyzeCircuit(self):
        ...
        i0 = numpy.array([1,2,3])  #define an initial guess for the currents in the circuit
...
class ResistorNetwork_2(ResistorNetwork):
    ...
    def AnalyzeCircuit(self):
        """
               ...
               """
        # need to set the currents to that Kirchoff's laws are satisfied
        i0 = numpy.array([1, 2, 3, 2, 1])   #define an initial guess for the currents in the circuit
        i = fsolve(self.GetKirchoffVals, i0)
        # print output to the screen
        print("I1 = {:0.1f}".format(i[0]))
        print("I2 = {:0.1f}".format(i[1]))
    ...
    def GetKirchoffVals(self,i):
        ...
        self.GetResistorByName('bc').Current = i[0]  # I_1 in diagram
        self.GetResistorByName('cd').Current = i[2]  # I_3 in diagram
        # set current in 4 ohm resistor in bottom loop.
        self.GetResistorByName('ce').Current = i[4]  # I_2 in diagram
        # set current in additional 5 ohm resistor.
        self.GetResistorByName('de').Current = i[3]  # I_2 in diagram
        # calculate net current into node e
        Node_c_Current = sum([i[4], i[3], -i[1]])

        KVL = self.GetLoopVoltageDrops()  # 3 equations here
        KVL.append(Node_c_Current)  # one equation here
        return KVL
