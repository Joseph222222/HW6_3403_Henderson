...
class rankine():
    ...
    def calc_efficiency(self):
        ...
        if(self.t_high==None):
            self.state1 = steam(pressure=self.p_high,x=1,name='Turbine Inlet') # instantiate a steam object with conditions of state 1 as saturated steam, named 'Turbine Inlet'
        else:
            self.state1 = steam(pressure=self.p_high,T=self.t_high,name='Turbine Inlet') # instantiate a steam object with conditions of state 1 at t_high, named 'Turbine Inlet'
        #state 2: turbine exit (p_low, s=s_turbine inlet) two-phase
        self.state2 = steam(pressure=self.p_low,s=self.state1.s,name='Turbine Exit') # instantiate a steam object with conditions of state 2, named 'Turbine Exit'
        #state 3: pump inlet (p_low, x=0) saturated liquid
        self.state3 = steam(pressure=self.p_low,x=0,name='Pump Inlet') # instantiate a steam object with conditions of state 3 as saturated liquid, named 'Pump Inlet'
        ...
        self.turbine_work = self.state1.h - self.state2.h # calculate turbine work (KJ/Kg) | work out (+)
        self.pump_work = self.state4.h - self.state3.h # calculate pump work | work in (KJ/Kg) (-) negative sign accounted for in eta calc. This variable should be positive.
        self.heat_added = self.state1.h - self.state4.h # calculate heat added | heat in (KJ/Kg) (+)
...
#region main
def main():
    rankine1 = rankine() #instantiate a rankine object to test it.
    #t_high is specified
