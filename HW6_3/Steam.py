    ...
    def calc(self):
        ...
        if self.T is not None:
            if self.T>Tsat: #interpolate with griddata
                ...
                self.h = float(griddata((tcol,pcol),hcol,(self.T,Pbar),'cubic'))  #use griddata to interpolate with T & P the superheated table
                self.s = float(griddata((tcol,pcol),scol,(self.T,Pbar),'cubic'))  #use griddata to interpolate with T & P the superheated table
                self.x=1.0
        ...
        elif self.h!=None: #Interpolate given an enthalpy and pressure value
            ...
            if self.x<=1.0: #manual interpolation
                ...
                self.region='Superheated'
                self.T = float(griddata((hcol,pcol),tcol,(self.h,Pbar),'cubic'))  #use griddata to interpolate with h & P the superheated table
                self.s = float(griddata((hcol,pcol),scol,(self.h,Pbar),'cubic'))  #use griddata to interpolate with h & P the superheated table
        elif self.s!=None: #Interpolate given an entropy and pressure value
            ...
            if self.x<=1.0: #manual interpolation
                ...
                self.region = 'Superheated'
                self.T = float(griddata((scol,pcol),tcol,(self.s,Pbar),'cubic'))  #use griddata to interpolate with s & P the superheated table
                self.h = float(griddata((scol,pcol),hcol,(self.s,Pbar),'cubic'))  #use griddata to interpolate with s & P the superheated table
        #endregion
